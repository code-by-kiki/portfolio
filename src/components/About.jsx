...
const skills = [
  { name: 'Core Java', level: 80,icon:<i class="bi bi-filetype-java"></i>  },
  { name: 'Java', level: 85,icon:<i class="bi bi-filetype-java" id="icons"></i> },
  { name: 'HTML', level: 95,icon:<i class="bi bi-filetype-html" id="icons"></i> },
  { name: 'CSS', level: 90 ,icon:<i class="bi bi-filetype-css" id="icons"></i> },
  { name: 'Bootstrap', level: 90,icon:<i class="bi bi-bootstrap" id="icons"></i> },
  { name: 'React', level: 85,icon:<i class="bi bi-filetype-jsx" id="icons"></i> },
  { name: 'MySQL', level: 75,icon:<i class="bi bi-filetype-sql" id="icons"></i> },
  { name: 'MangoDb', level: 65,icon:<i class="bi bi-leaf" id="icons"></i> },
   { name: 'GitHub', level: 90,icon:<i class="bi bi-github" id="icons"></i> },
];
...
const SkillCard = ({ name, level,icons }) => (
  <motion.div className="col-md-4 mb-4" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: false }}>
    <div className="card shadow-sm border-1">
      <div className="card-body text-center">
        <h1 style={{color:'rgb(217, 125, 85)'}}>{icons} </h1>
        <h5 className="card-title">{name}</h5>
        <div className="progress" style={{ height: '8px' }}>
          <motion.div
            className="progress-bar"
            style={{backgroundColor:"rgb(111, 164, 175)"}}
            role="progressbar"
            initial={{ width: 0 }}
            animate={{ width: `${level}%` }}
        ...
        <small className=" text-drak mt-2 d-block">{level}%</small>
      </div>
    </div>
  </motion.div>
);
...
const AboutData = () => (  
  <div className="container py-5">
     <div className="text-2xl font-bold ">
       <p className=' d-none d-md-block' style={aboutme}>I’m Krithika, a passionate and driven graduate with a Bachelor’s degree in Economics and a strong foundation in technology. I have successfully completed a comprehensive course in Java Full Stack Development, equipping me with hands-on experience in building dynamic web applications using modern frameworks and tools.
    ...
    <div className="row ">
      {skills.map((skill, index) => (
        <SkillCard key={index} name={skill.name} level={skill.level} icons={skill.icon} />
      ))}
